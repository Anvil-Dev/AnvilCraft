...
public class Storages extends SavedData {
    ...
    public static final MapCodec<Storages> CODEC = CodecUtil.mapCodec(
        Codec.unboundedMap(UUIDUtil.CODEC, BaseStorage.CODEC.codec())
            .fieldOf("storages")
    ...
    public static Storages get() {
        if (Util.isServer()) {
            ...
            throw new IllegalStateException("Cannot access Storages on client");
        }
    }
    ...
    public <T extends BaseStorage> T getOrCreate(UUID id, Class<T> clazz) {
        AtomicReference<@Nullable BaseStorage> storage = new AtomicReference<>(this.storages.get(id));
        if (storage.get() == null) {
            storage.set(StorageType.find(clazz).newInstance());
            this.storages.put(id, storage.get());
        ...
        return Util.castSafely(storage, clazz).orElseThrow(() -> new IllegalArgumentException(
            "Storage with id '%s' cannot be cast to expected type '%s'. Actual type: %s"
                .formatted(id, clazz.getName(), storage.get().getClass().getName())
        ));
    }
}
