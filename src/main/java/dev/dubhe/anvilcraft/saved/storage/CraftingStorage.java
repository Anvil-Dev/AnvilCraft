...
public record CraftingStorage(
    ...
    public CraftingStorage withStonecutterInput(ItemStack input) {
        return new CraftingStorage(input, this.craftingInput, this.stonecutterSelected, this.lastOpened);
    }

    /** ② 合成 9 宫格（整体替换）。 */
    public CraftingStorage withCraftingInput(List<ItemStack> input) {
        return new CraftingStorage(this.stonecutterInput, input, this.stonecutterSelected, this.lastOpened);
    }
    ...
    public CraftingStorage withCraftingSlot(int slot, ItemStack stack) {
        ...
        grid.set(slot, stack);
        return new CraftingStorage(this.stonecutterInput, grid, this.stonecutterSelected, this.lastOpened);
    }

    /** 切石机选中配方索引。 */
    public CraftingStorage withStonecutterSelected(int selected) {
        return new CraftingStorage(this.stonecutterInput, this.craftingInput, selected, this.lastOpened);
    }

    /** 上次关闭界面时是否为合成模式。 */
    public CraftingStorage withLastOpened(boolean opened) {
        return new CraftingStorage(this.stonecutterInput, this.craftingInput, this.stonecutterSelected, opened);
    }
}
