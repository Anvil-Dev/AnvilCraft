...
public final class VanillaBuildingEntities {
    ...
    private static final class VehicleAdapter implements EntityBuildAdapter {
        ...
        public boolean matches(EntityType<?> type, CompoundTag nbt) {
            ...
            return AbstractBoat.class.isAssignableFrom(base) || AbstractMinecart.class.isAssignableFrom(base);
        }

        @Override
        public Planned plan(ServerLevel level, StructureSnapshot.EntityEntry entry, CompoundTag transformedNbt) {
            EntityType<?> type = EntityBuildAdapters.typeOf(transformedNbt).orElse(null);
            ...
            sanitized.putString("id", entity == null
                ? transformedNbt.getStringOr("id", "") : EntityType.getKey(entity.getType()).toString());
            List<FluidBuildAdapter.TankFluid> fluids = new ArrayList<>();
            if (entity != null) {
                ResourceHandler<FluidResource> handler = fluidHandlerOf(entity);
                if (handler != null) {
                    for (int tank = 0; tank < handler.size(); tank++) {
                        if (!handler.getResource(tank).toStack(handler.getAmountAsInt(tank)).isEmpty()) {
                            fluids.add(new FluidBuildAdapter.TankFluid(tank,
                                handler.getResource(tank).toStack(handler.getAmountAsInt(tank))));
                        }
                    }
                }
    ...
    private static ItemStack vehicleItem(@Nullable EntityType<?> type) {
        ...
        if (type == EntityType.OAK_CHEST_BOAT) {
    ...
    @Nullable
    private static ResourceHandler<FluidResource> fluidHandlerOf(Entity entity) {
        if (entity instanceof IFluidResourceHandlerHolder holder) {
            return holder.getFluidHandler();
        }
        return entity.getCapability(Capabilities.Fluid.ENTITY, null);
    }
}
