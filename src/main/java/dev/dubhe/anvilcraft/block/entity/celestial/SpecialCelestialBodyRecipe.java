...
public record SpecialCelestialBodyRecipe(
    ...
    public SpecialCelestialBodyRecipe(
        ...
        this(name, model, needsCustomModel, time, space, mass, energy, hasAtmosphere, liquidCoverage,
            magneticFieldStrength, rotationSpeed, axialTilt, seedItems, minerals, fluids, biologicalItems,
            List.of(), offerings, templeBlessings, templePunishments);
    }
    ...
    private static final Codec<Identifier> RESOURCE_LOCATION_CODEC =
        Identifier.CODEC;
    ...
    private static final Codec<ColorRGBA> ATMOSPHERE_COLOR_CODEC = Codec.STRING.comapFlatMap(value -> {
        if (!value.startsWith("#")) return DataResult.error(() -> "Not a color code: " + value);
        try {
            return DataResult.success(new ColorRGBA((int) Long.parseLong(value.substring(1), 16)));
        } catch (NumberFormatException exception) {
            return DataResult.error(() -> "Exception parsing color code: " + exception.getMessage());
        }
    }, color -> String.format(java.util.Locale.ROOT, "#%08X", color.rgba()));
    ...
    private record AtmosphereFields(Optional<ColorRGBA> atmosphere, Optional<Boolean> hasAtmosphere) {
        static final MapCodec<AtmosphereFields> CODEC = RecordCodecBuilder.mapCodec(ins -> ins.group(
            ATMOSPHERE_COLOR_CODEC.optionalFieldOf("atmosphere").forGetter(AtmosphereFields::atmosphere),
    ...
    private static SpecialCelestialBodyRecipe fromCodec(
        ...
        List<Identifier> seedItems, BehaviorFields behavior,
    ...
    private static boolean legacyCanBeShattered(Optional<CelestialTravelData> landing) {
        return landing.map(CelestialTravelData::dimension)
            .filter(CelestialTravelData.OVERWORLD_LIKE_DIMENSION::equals)
            .isPresent();
    }
    ...
    public static final RecipeSerializer<SpecialCelestialBodyRecipe> SERIALIZER = new RecipeSerializer<>(CODEC, STREAM_CODEC);

    /// === 派生属性 ===

    public boolean hasAtmosphere() {
        return this.atmosphere.isPresent();
    }

    /// 温度由 energy 砧子数自动推导。
    public Temperature temperature() {
        return CelestialBodyMatcher.energyToTemperature(this.energy);
    }
    ...
    public PlanetaryResourceSet generateResources() {
        ...
        for (WeightedEntry entry : this.biologicalFluids) {
