...
public class TeslaGunItem extends Item {
    ...
    public TeslaGunItem(Properties properties) {
        super(properties.component(ModComponents.STORED_ENERGY, new StoredEnergy(TeslaGunItem.MAX_ENERGY)));
    }
    ...
    public int getBarWidth(ItemStack stack) {
        ...
        return Math.round(Math.clamp((float) energy / CorruptedBeaconActivatorItem.MAX_ENERGY, 0, 1) * 13);
    }

    @Override
    public int getBarColor(ItemStack stack) {
        float energy = stack.getOrDefault(ModComponents.STORED_ENERGY, StoredEnergy.EMPTY).value();
        return ColorUtil.lerpColor(energy / CorruptedBeaconActivatorItem.MAX_ENERGY, BAR_COLOR, FULL_BAR_COLOR);
    }
}
