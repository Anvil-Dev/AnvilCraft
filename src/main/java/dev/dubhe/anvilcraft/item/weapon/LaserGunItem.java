...
public class LaserGunItem extends Item {
    ...
    public LaserGunItem(Properties properties) {
        super(properties.component(ModComponents.STORED_ENERGY, LaserGunItem.MAX_ENERGY));
    }
    ...
    public int getBarWidth(ItemStack stack) {
        ...
        return Math.round(Math.clamp((float) energy / LaserGunItem.MAX_ENERGY, 0, 1) * 13);
    }

    @Override
    public int getBarColor(ItemStack stack) {
        float energy = stack.getOrDefault(ModComponents.STORED_ENERGY, 0);
        return ColorUtil.lerpColor(energy / LaserGunItem.MAX_ENERGY, BAR_COLOR, FULL_BAR_COLOR);
    }
}
