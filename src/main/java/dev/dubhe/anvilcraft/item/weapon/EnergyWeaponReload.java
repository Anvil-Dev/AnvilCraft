...
public final class EnergyWeaponReload {
    ...
    public static boolean isReloading(Player player, ItemStack stack) {
        if (player.level().isClientSide()) {
            ...
            int slot = data.getIntOr(CLIENT_SLOT, -1);
    ...
    public static void login(PlayerEvent.PlayerLoggedInEvent event) {
        ...
        ItemStack returned = ItemStack.OPTIONAL_CODEC.parse(player.registryAccess().createSerializationContext(NbtOps.INSTANCE),
            data.get(ESCROW)).result().orElse(ItemStack.EMPTY);
        data.remove(ESCROW);
    ...
    private static final class Session {
        ...
        private Session(
            ...
            this.inventorySlot = this.hand == InteractionHand.MAIN_HAND ? player.getInventory().getSelectedSlot() : Inventory.SLOT_OFFHAND;
        }
        ...
        private void saveEscrow(ServerPlayer player) {
            player.getPersistentData().put(ESCROW, ItemStack.OPTIONAL_CODEC.encodeStart(
                player.registryAccess().createSerializationContext(NbtOps.INSTANCE), this.returned).getOrThrow());
        }
