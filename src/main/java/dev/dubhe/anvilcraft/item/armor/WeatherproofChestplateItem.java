...
public class WeatherproofChestplateItem extends IonoCraftBackpackItem implements ICapacitorChargeable {
    ...
    public WeatherproofChestplateItem(Properties properties) {
        super(properties.component(ModComponents.STORED_ENERGY, StoredEnergy.EMPTY), true);
    }
    ...
    public static int getEnergyStored(ItemStack stack) {
        return Math.clamp(stack.getOrDefault(ModComponents.STORED_ENERGY, StoredEnergy.EMPTY).value(), 0, MAX_ENERGY);
    }
    ...
    public static void tickEnergy(ServerPlayer player) {
        ...
        if (player.getAbilities().flying) {
            stack.set(ModComponents.STORED_ENERGY, new StoredEnergy(Math.max(0, getEnergyStored(stack) - FLIGHT_CONSUMPTION)));
        }
