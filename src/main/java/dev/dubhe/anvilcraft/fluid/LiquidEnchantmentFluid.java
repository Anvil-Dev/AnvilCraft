...
/// 液态魔咒只能存在于流体容器中，无法放置到世界或装入桶中。
public class LiquidEnchantmentFluid extends Fluid {
    public static final FluidType TYPE = new FluidType(FluidType.Properties.create()
        .descriptionId("block.anvilcraft.liquid_enchantment")
    ) {
        @Override
        public Component getDescription(FluidStack stack) {
            return LiquidEnchantmentUtil.getEnchantment(stack)
                .<Component>map(enchantment -> Component.translatable(
                    "block.anvilcraft.liquid_enchantment.enchanted",
