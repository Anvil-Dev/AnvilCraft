...
public class WeaponBeamEntity extends Entity {
    ...
    private static final EntityDataAccessor<Float> END_X = SynchedEntityData.defineId(WeaponBeamEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> END_Y = SynchedEntityData.defineId(WeaponBeamEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Float> END_Z = SynchedEntityData.defineId(WeaponBeamEntity.class, EntityDataSerializers.FLOAT);
    private static final EntityDataAccessor<Integer> STYLE = SynchedEntityData.defineId(WeaponBeamEntity.class, EntityDataSerializers.INT);
    ...
    public void tick() {
        super.tick();
        if (this.level().isClientSide && getStyle() != TESLA) return;
        if (this.tickCount > (getStyle() == TESLA ? 5 : 2)) this.discard();
    }

    @Override
    protected void defineSynchedData(SynchedEntityData.Builder builder) {
        builder.define(END_X, 0.0F).define(END_Y, 0.0F).define(END_Z, 0.0F)
            .define(STYLE, CORRUPTED).define(STRENGTH, 1).define(OWNER_ID, -1);
    }
