...
public abstract class AutoCrafterMenu extends BaseMachineMenu implements IFilterMenu {
    public static AutoCrafterMenu clientOf(int containerId, Inventory inventory) {
        return new Client(containerId, inventory);
    }
    ...
    private static class Server extends AutoCrafterMenu {
        ...
        @Override
        public void clicked(int slotId, int button, ClickType clickType, Player player0) {
    ...
    private static class Client extends AutoCrafterMenu {
        public Client(int containerId, Inventory inventory) {
            super(containerId, inventory, new AutoCrafterContainer(NonNullList.withSize(9, ItemStack.EMPTY)));
        }
    }
}
