...
public class TranscendenceSmithingMenu extends AbstractContainerMenu {
    ...
    public TranscendenceSmithingMenu(
        ...
        this.royalRecipes = this.level.getRecipeManager().getAllRecipesFor(RecipeType.SMITHING);
        this.emberRecipes = this.level.getRecipeManager()
            .getAllRecipesFor(ModRecipeTypes.MULTIPLE_TO_ONE_SMITHING_TYPE.get());
    ...
    private List<RecipeHolder<? extends IFrostSmithingRecipe>> collectFrostRecipes() {
        ...
        result.addAll(this.level.getRecipeManager().getAllRecipesFor(ModRecipeTypes.PERMUTATION_TYPE.get()));
        result.addAll(this.level.getRecipeManager().getAllRecipesFor(ModRecipeTypes.DEFORMATION_TYPE.get()));
    ...
    /** 处理模板选择或置顶请求。 */
    public void handleTemplateAction(Player player, ResourceLocation templateId, boolean toggleFavorite) {
    ...
    private void createRoyalResult() {
        ...
        List<RecipeHolder<SmithingRecipe>> matches = this.level.getRecipeManager()
    ...
    private void createEmberResult() {
        ...
        List<RecipeHolder<BaseMultipleToOneSmithingRecipe>> matches = this.level.getRecipeManager()
            .getRecipesFor(ModRecipeTypes.MULTIPLE_TO_ONE_SMITHING_TYPE.get(), input, this.level);
        if (matches.isEmpty()) return;
    ...
    private void createFrostResult() {
        ...
        if (this.selectedTemplate.getItem() instanceof PermutationTemplateItem) {
            List<RecipeHolder<PermutationRecipe>> matches = this.level.getRecipeManager()
        ...
        List<RecipeHolder<DeformationRecipe>> matches = this.level.getRecipeManager()
    ...
    private void setFrostResult(
        ...
        if (options.stream().anyMatch(option -> !option.result().result().isEnabled(this.level.enabledFeatures()))) {
            return;
        }
    ...
    private void consumeItems(Container container, @SuppressWarnings("SameParameterValue") int index, int count) {
        if (count <= 0) return;
        ItemStack stack = container.getItem(index);
        if (stack.isEmpty()) return;
        stack.shrink(count);
