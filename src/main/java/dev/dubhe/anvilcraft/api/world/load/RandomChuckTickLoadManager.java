...
public class RandomChuckTickLoadManager {
    ...
    public static void register(BlockPos centerPos, LoadChuckData loadChuckData) {
        if (RandomChuckTickLoadManager.RANDOM_TICK_LOAD_CHUCK_AREA_MAP.containsKey(centerPos)) {
            RandomChuckTickLoadManager.unregister(centerPos);
        }
        RandomChuckTickLoadManager.RANDOM_TICK_LOAD_CHUCK_AREA_MAP.put(centerPos, loadChuckData);
    }

    public static void unregister(BlockPos centerPos) {
        if (!RandomChuckTickLoadManager.RANDOM_TICK_LOAD_CHUCK_AREA_MAP.containsKey(centerPos)) return;
        RandomChuckTickLoadManager.RANDOM_TICK_LOAD_CHUCK_AREA_MAP.remove(centerPos);
    }

    /// tick
    public static void tick() {
        for (LoadChuckData loadChuckData : RandomChuckTickLoadManager.RANDOM_TICK_LOAD_CHUCK_AREA_MAP.values()) {
