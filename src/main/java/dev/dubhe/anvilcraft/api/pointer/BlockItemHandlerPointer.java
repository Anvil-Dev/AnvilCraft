...
public class BlockItemHandlerPointer implements ITargetPointer {
    ...
    private @Nullable ResourceHandler<ItemResource> resolveHandler(Level level) {
    ...
    private boolean matchesStoredStack(ResourceHandler<ItemResource> handler) {
        return this.slot >= 0
            && this.slot < handler.size()
            && ItemStack.isSameItemSameComponents(handler.getResource(this.slot).toStack(handler.getAmountAsInt(this.slot)), this.stack);
    }
    ...
    private boolean placeToPos(ServerLevel level, BlockPos pos, @Nullable BlockState requiredState) {
        ...
        if (!returnItem.isEmpty()) {
            ...
            try (Transaction transaction = Transaction.openRoot()) {
                ItemResource resource = ItemResource.of(returnStack);
                int remaining = returnStack.getCount();
                for (int i = 0; i < handler.size() && remaining > 0; i++) {
                    remaining -= handler.insert(i, resource, remaining, transaction);
                }
                transaction.commit();
            }
        }
    ...
    private int extractMatchingItems(ResourceHandler<ItemResource> handler, int amount, boolean simulate) {
        ItemResource resource = ItemResource.of(this.stack);
        try (Transaction transaction = Transaction.openRoot()) {
            int extractedCount = 0;
            for (int slot = 0; slot < handler.size() && extractedCount < amount; slot++) {
                if (!resource.equals(handler.getResource(slot))) continue;
                extractedCount += handler.extract(slot, resource, amount - extractedCount, transaction);
            }
    ...
    public static class Type implements ITargetPointer.Type<BlockItemHandlerPointer> {
        public static final Codec<Type> CODEC = ModRegistries.TARGET_POINTER_TYPE.byNameCodec().flatXmap(
            raw -> raw instanceof Type pointerType
                   ? DataResult.success(pointerType)
        ...
        public @Nullable BlockItemHandlerPointer point(
            ...
            ResourceHandler<ItemResource> handler = ItemHandlerUtil.getSourceItemHandler(pos, facing, level);
            if (handler == null) {
                return null;
            }
            Predicate<ItemStack> filter = this.filter == null ? stack -> true : this.filter;
            for (int i = 0; i < handler.size(); i++) {
                ItemStack inSlot = handler.getResource(i).toStack(handler.getAmountAsInt(i));
