...
public class SuperHeatingBehavior implements AnvilBehavior {
    ...
    public void handle(Level level, BlockPos hitBlockPos, BlockState hitBlockState, float fallDistance, AnvilFallOnLandEvent event) {
        ...
        if (belowState.is(ModBlocks.HEATER) && !belowState.getValue(HeaterBlock.OVERLOAD)) {
            ...
            level.getRecipeManager().getRecipeFor(ModRecipeTypes.SUPER_HEATING_TYPE.get(), input, level).ifPresent(recipe -> {
                ...
                for (int i = 0; i < times; i++) {
                    ...
                    if (recipe.value().blockResult != null) {
                        level.setBlockAndUpdate(hitBlockPos, recipe.value().blockResult.defaultBlockState());
                    }
                }
