...
public record RepairMaterialFrostMaterialPredicate(
    ...
    /**
     * 默认通用材料消耗为维修材料消耗的两倍；配方可用表达式或数据包函数引用覆盖。
     */
    ...
    public boolean test(IFrostSmithingRecipe recipe, FrostSmithingRecipeInput input) {
        if (this.matchesRepairMaterial(recipe, input)) {
            return input.material().getCount() >= this.cost;
        }
        if (this.matchesUniversalMaterial(input)) {
            return input.material().getCount() >= this.universalCostAmount();
        }
        return false;
    }
    ...
    private boolean matchesRepairMaterial(IFrostSmithingRecipe recipe, FrostSmithingRecipeInput input) {
        ...
        if (!equipment.isEmpty()) {
            return isRepairMaterial(equipment, material);
        }
    ...
    private static boolean isRepairMaterial(ItemStack equipment, ItemStack material) {
        return !equipment.isEmpty() && equipment.isValidRepairItem(material);
    }
