...
public class MDAnvilCollisionCraftRecipeComponent extends MDRecipeComponent {
    ...
    public static final Identifier TEXTURE =
        Identifier.fromNamespaceAndPath("anvilcraft", "textures/gui/ageratum/256back.png");
    public static final Identifier EXPLOSION =
        Identifier.fromNamespaceAndPath("anvilcraft", "textures/gui/jei/explosion.png");
    ...
    protected void extractRecipeRenderState(MDRenderContext context, float mouseX, float mouseY) {
        ...
        if (!this.recipe.outputItems().isEmpty()) {
            ...
            AgeratumUtil.renderItems(context, this.recipe.outputItems(), mouseX, mouseY, OUTPUT_ITEM_X, OUTPUT_ITEM_Y);
        }
        ...
        if (!this.recipe.transformBlocks().isEmpty()) {
            ...
            for (BlockTransform blockTransform : blockTransforms) {
                AgeratumUtil.renderBlock(context, blockTransform.inputBlock(), mouseX, mouseY, TRANSFORM_X, TRANSFORM_INPUT_Y);
                AgeratumUtil.renderArrow(guiGraphics, TRANSFORM_ARROW_X, TRANSFORM_ARROW_Y, 90);
                AgeratumUtil.renderBlock(context, blockTransform.outputBlock(), mouseX, mouseY, TRANSFORM_X, TRANSFORM_OUTPUT_Y);
