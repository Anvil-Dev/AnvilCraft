...
public class RedstoneDiceInputListener {
    ...
    public static void onMouseButton(InputEvent.MouseButton.Post event) {
        if (!Minecraft.getInstance().options.keyUse.matchesMouse(new MouseButtonEvent(0, 0, event.getMouseButtonInfo()))) return;
    ...
    public static void onKey(InputEvent.Key event) {
        if (!Minecraft.getInstance().options.keyUse.matches(event.getKeyEvent())) return;
        if (event.getAction() == GLFW.GLFW_PRESS) press();
        if (event.getAction() == GLFW.GLFW_RELEASE) release();
    }
    ...
    private static WheelMenuModel buildWheel(BlockPos pos) {
        ...
        for (boolean uniform : new boolean[]{true, false}) {
            ...
            builder.action(mode, label, (graphics, pose, width, height) -> graphics.item(icon, -8, -8),
                context -> ClientPacketDistributor.sendToServer(new RedstoneDiceModePacket(pos, uniform)));
        }
    ...
    private static void release() {
        ...
        if (wheelScreen != null) {
            ...
            ClientPacketDistributor.sendToServer(new RedstoneDiceRollPacket(heldPos, heldHand));
        }
        reset();
    }
