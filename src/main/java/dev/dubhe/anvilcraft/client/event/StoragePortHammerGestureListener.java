...
public class StoragePortHammerGestureListener {
    ...
    public static void onMouseButton(InputEvent.MouseButton.Post event) {
        ...
        if (client.options.keyUse.getKey().getType() != InputConstants.Type.MOUSE
            || client.options.keyUse.getKey().getValue() != event.getButton()) {
            return;
        }
    ...
    private static WheelMenuModel buildClearMarkWheel(BlockPos pos) {
        return WheelMenuBuilder.create()
            ...
            .action(
                ...
                (graphics, pose, width, height) -> graphics.item(new ItemStack(Items.BARRIER), -8, -8),
                ctx -> {
                    ClientPacketDistributor.sendToServer(new StoragePortUnmarkPacket(pos));
