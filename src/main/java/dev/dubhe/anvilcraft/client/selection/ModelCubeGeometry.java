...
public final class ModelCubeGeometry {
    // 延续源分支的局部坐标精度，放大薄面后再由 SelectionPart 还原。
    ...
    public static List<ConvexShape> decode(List<CuboidModelElement> elements) {
        if (elements.isEmpty()) return List.of();
        boolean hasVolume = elements.stream().anyMatch(element -> !isPlane(element));
        List<CuboidModelElement> scaled = new ArrayList<>(elements.size());
        for (CuboidModelElement element : elements) {
            ...
            if (rotation != null) {
                rotation = new CuboidRotation(
                    new Vector3f(rotation.origin()).mul(SCALE), rotation.value(), rotation.rescale(), rotation.transform()
                );
            }
            scaled.add(new CuboidModelElement(
                from.mul(SCALE), to.mul(SCALE), element.faces(), rotation, element.shade(), element.lightEmission(), element.faceData()
            ));
        }
        return CubeModelDecoder.decode(scaled, BlockModelRotation.IDENTITY);
    }

    private static boolean isPlane(CuboidModelElement element) {
