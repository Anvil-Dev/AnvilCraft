...
public final class ModelBlockSelection {
    ...
    private static volatile Snapshot pending = new Snapshot(Map.of(), Map.of(), Map.of());
    ...
    public record ModelPlacement(SelectionModel model, Matrix4f pose) {
    }
    ...
    public static void reload(ModelEvent.BakingCompleted event) {
        snapshot = pending;
        pending = new Snapshot(Map.of(), Map.of(), Map.of());
        DYNAMIC.clear();
    ...
    private static <T extends BlockEntity> List<SelectionPart> rendererParts(
        ...
        ModelSelectionRenderer.ModelConsumer consumer = (model, pose) -> {
            ModelSelection selection = snapshot.models().get(model);
            for (SelectionPart source : ModelSelectionBakery.collect(selection, entity.getBlockState().getSeed(entity.getBlockPos()))) {
                pose.pushPose();
                source.apply(pose);
    ...
    private static List<OutlinePart> prepareOutline(List<SelectionPart> parts) {
        List<OutlinePart> result = new ArrayList<>();
        int segments = 0;
        AABB bounds = null;
        for (SelectionPart part : parts) {
            PackedOutline outline = CubeSelection.outlines().get(part.geometry());
            result.add(new OutlinePart(part, outline));
            segments += outline.segmentCount();
            bounds = bounds == null ? part.bounds() : bounds.minmax(part.bounds());
        }
        if (segments > SelectionGeometry.MAX_OUTLINE_SEGMENTS && bounds != null) {
            SelectionGeometry geometry = new SelectionGeometry(List.of(ConvexShape.box(bounds)));
            result.clear();
            result.add(new OutlinePart(new SelectionPart(geometry), geometry.fallbackOutline()));
        }
        return result;
    }

    private static <P extends Enum<P>> void collectDynamicOutline(
        AbstractMultiPartBlock<P> block, BlockState state, BlockPos pos, ClientLevel level, float partialTick, List<OutlinePart> output
    ) {
        for (P part : block.getParts()) {
            ...
            for (SelectionPart dynamic : dynamic(level, partPos, partialTick)) {
                PoseStack pose = new PoseStack();
                pose.translate(partPos.getX() - pos.getX(), partPos.getY() - pos.getY(), partPos.getZ() - pos.getZ());
                dynamic.apply(pose);
                output.add(new OutlinePart(new SelectionPart(dynamic.geometry(), pose.last().pose()),
                    CubeSelection.outlines().get(dynamic.geometry())));
            }
        }
    }
    ...
    private record OutlinePart(SelectionPart part, PackedOutline outline) {
    }

    record Snapshot(Map<BlockState, ModelSelection> states, Map<BlockState, List<SelectionPart>> outlines,
                    Map<SelectionModel, ModelSelection> models) {
    }
}
