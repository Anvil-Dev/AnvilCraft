...
import dev.dubhe.anvilcraft.mixin.PocketRecipeScreenAccessor;
...
public final class PocketEffectLayout {
    ...
    public static boolean isHiddenByRecipeBook(AbstractContainerScreen<?> screen) {
        return screen instanceof InventoryScreen && screen instanceof PocketRecipeScreenAccessor book
            && book.anvilcraft$isWidthTooNarrow() && book.anvilcraft$getRecipeBookComponent().isVisible();
    }
    ...
    public static List<Rect2i> areas(AbstractContainerScreen<?> screen, int count) {
        if (isHiddenByRecipeBook(screen)) return List.of();
        return areas(screen.width, screen.getLeftPos(), screen.getTopPos(), screen.getImageWidth(),
