...
public final class EnergyWeaponFirstPersonRenderer {
    ...
    private static final Matrix4f WORLD_VIEW = new Matrix4f();
    private static final Matrix4f HAND_PROJECTION = new Matrix4f();
    private static final Matrix4f HAND_VIEW = new Matrix4f();
    ...
    public static void captureBeam(PoseStack poseStack, Vec3 end, float partialTick) {
        ...
        Matrix4f projection = new Matrix4f(WORLD_PROJECTION)
            .mul(WORLD_VIEW).mul(poseStack.last().pose());
    ...
    public static void worldProjection(Matrix4fc projection) {
        WORLD_PROJECTION.set(projection);
        WORLD_VIEW.set(Minecraft.getInstance().gameRenderer.getGameRenderState().levelRenderState.cameraRenderState.viewRotationMatrix);
    }

    public static void beginHand(CameraRenderState camera, Matrix4fc handProjection, Matrix4fc view) {
        HAND_PROJECTION.set(handProjection);
        HAND_VIEW.set(view);
        Minecraft minecraft = Minecraft.getInstance();
        ...
        if (minecraft.options.bobView().get()) WeaponBeamRenderer.counterViewBob(pose, camera);
        Matrix4f projection = new Matrix4f(WORLD_PROJECTION).mul(view).mul(pose.last().pose());
    ...
    public static void renderHand(RenderHandEvent event) {
        ...
        DRAW_ANIMATIONS[InteractionHand.MAIN_HAND.ordinal()].observe(player.getMainHandItem(), player.getInventory().getSelectedSlot());
    ...
    private static boolean isReloadStillHeld(AbstractClientPlayer player) {
        return (reloadSlot == Inventory.SLOT_OFFHAND || reloadSlot == player.getInventory().getSelectedSlot())
    ...
    private static void cameraSpace(PoseStack pose) {
        Matrix4f camera = HAND_VIEW.invert(new Matrix4f());
    ...
    private static Vector3f unprojectAtDepth(Vector3f projected, float depth) {
        Matrix4f projection = HAND_PROJECTION;
    ...
    private static void renderReload(RenderHandEvent event, AbstractClientPlayer player) {
        ...
        if (EnergyWeaponReloadPose.showsCapacitor(elapsed)) {
            ...
            renderItem(reloadCapacitor, pose, event.getSubmitNodeCollector(), event.getPackedLight(), player);
            pose.popPose();
    ...
    private static void renderWeapon(
        ...
        pose.pushPose();
        pose.mulPose(weapon);
        renderItem(stack, pose, event.getSubmitNodeCollector(), event.getPackedLight(), player);
        pose.popPose();
    ...
    private static void renderArm(RenderHandEvent event, AbstractClientPlayer player, HumanoidArm arm, Vector3f grip) {
        ...
        float side = arm == HumanoidArm.RIGHT ? 1 : -1;
        EnergyWeaponArmRenderer.render(event.getPoseStack(), event.getSubmitNodeCollector(), event.getPackedLight(), player, arm,
            EnergyWeaponReloadPose.arm(grip, side));
    }

    private static void renderItem(ItemStack stack, PoseStack pose, SubmitNodeCollector collector, int light, AbstractClientPlayer player) {
        var state = new ItemStackRenderState();
        Minecraft.getInstance().getItemModelResolver().updateForTopItem(
            state, stack, ItemDisplayContext.NONE, player.level(), player, player.getId());
