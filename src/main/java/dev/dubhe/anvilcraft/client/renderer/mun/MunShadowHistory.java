...
final class MunShadowHistory implements AutoCloseable {
    ...
    private int read;
    private int savedHistoryTexture;
    private int savedHistorySampler;
    ...
    void begin() {
        ...
        if (!MunShadowGlScope.supported() || !GL.getCapabilities().GL_ARB_shader_image_load_store) return;
        if (this.active) this.end();
        if (this.frame == FRAME_LIMIT - 1) this.close();
        final int activeTexture = GL30C.glGetInteger(GL30C.GL_ACTIVE_TEXTURE);
        GlStateManager._activeTexture(GL30C.GL_TEXTURE6);
        this.savedHistoryTexture = GL30C.glGetInteger(GL30C.GL_TEXTURE_BINDING_2D);
        this.savedHistorySampler = GL30C.glGetIntegeri(GL33C.GL_SAMPLER_BINDING, 6);
        ...
        try (MemoryStack stack = MemoryStack.stackPush()) {
            ...
            try {
                ...
                GlStateManager._colorMask((mask.get(0) != 0 ? 1 : 0) | (mask.get(1) != 0 ? 2 : 0)
    ...
    void end() {
        ...
        final int activeTexture = GL30C.glGetInteger(GL30C.GL_ACTIVE_TEXTURE);
        GlStateManager._activeTexture(GL30C.GL_TEXTURE6);
        GlStateManager._bindTexture(this.savedHistoryTexture);
        GL33C.glBindSampler(6, this.savedHistorySampler);
        GlStateManager._activeTexture(activeTexture);
    ...
    public void close() {
        ...
        for (int index = 0; index < 2; index++) {
            if (this.textures[index] != 0) GlStateManager._deleteTexture(this.textures[index]);
            this.textures[index] = 0;
        }
        if (this.claims != 0) GlStateManager._deleteTexture(this.claims);
