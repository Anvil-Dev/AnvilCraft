...
public final class MunSurfaceRenderer {
    ...
    private static final Map<RenderPipeline, RenderPipeline> SHADOW_PIPELINES = new IdentityHashMap<>();
    private static final Map<RenderPipeline, RenderPipeline> HISTORY_PIPELINES = new IdentityHashMap<>();
    private static final MunShadowReceiver SHADOWS = new MunShadowReceiver();
    private static MunLightingQuality quality = MunLightingQuality.OFF;
    ...
    public static void tick(ClientTickEvent.Post event) {
        ...
        var nextQuality = AnvilCraft.CLIENT_CONFIG.munLightingQuality;
        if (quality != nextQuality) {
            SHADOWS.close();
            compiled = false;
            quality = nextQuality;
            profile = MunLightingProfile.of(quality);
        }
        boolean next = client.level != null && CelestialTravelManager.MUN_LEVEL.equals(client.level.dimension())
            && quality != MunLightingQuality.OFF && !IrisState.isShaderEnabled();
        if (next && !compiled) {
            try {
                historyEnabled = MunShadowGlScope.supported() && GL.getCapabilities().GL_ARB_shader_image_load_store;
                ...
                for (var pipeline : REPLACEMENTS) {
                    boolean shadows = pipeline.getShaderDefines().flags().contains("MUN_SHADOWS");
                    boolean history = pipeline.getShaderDefines().flags().contains("MUN_SHADOW_HISTORY");
                    if (shadows != (profile.cascades() > 0) || shadows && history != historyEnabled) continue;
    ...
    public static void extract(ExtractLevelRenderStateEvent event) {
        ...
        Vec3 anchor = entity == null ? camera : entity.getPosition(partial).add(0, 1.6, 0);
        event.getRenderState().setRenderData(FRAME, new Frame(event.getLevel().getOverworldClockTime(),
            MunClientSky.partialDayTime(event.getLevel(), partial), partial, camera, anchor,
    ...
    public static void beginWorld(LevelRenderState state) {
        ...
        worldPass = frame != null && AnvilCraft.CLIENT_CONFIG.munLightingQuality != MunLightingQuality.OFF;
        if (frame == null || !worldPass) return;
        if (uniforms == null) {
            uniforms = RenderSystem.getDevice().createBuffer(() -> "Mun surface uniforms",
                GpuBuffer.USAGE_COPY_DST | GpuBuffer.USAGE_UNIFORM, 384);
        }
        Vec3 origin = Vec3.atLowerCornerOf(BlockPos.containing(profile.cascades() > 0 ? frame.anchor : frame.camera));
        SOLAR.update(frame.time, frame.partialTime, origin);
        if (profile.cascades() > 0 && Minecraft.getInstance().level != null) {
            try {
                SHADOWS.begin(Minecraft.getInstance().level, frame.time, frame.partialTime, frame.partialTick,
                    frame.anchor, origin, SOLAR, profile, historyEnabled);
            } catch (RuntimeException exception) {
                SHADOWS.close();
                MunRenderPipeline.fail(exception);
                worldPass = false;
        ...
        try (var stack = MemoryStack.stackPush()) {
            ...
            SOLAR.write(bytes, frame.camera, profile.ambientFloor());
    ...
    public static void post(net.neoforged.neoforge.client.event.RenderLevelStageEvent.AfterOpaqueFeatures event) {
        ...
        if (worldPass && frame != null) {
            try {
                POST.render(frame.projection, profile, frame.ambientOcclusion);
            } catch (RuntimeException exception) {
                MunRenderPipeline.fail(exception);
                worldPass = false;
            }
    ...
    public static RenderPipeline replace(RenderPipeline original) {
        ...
        return variant(original, profile.cascades() > 0, historyEnabled);
    }

    private static RenderPipeline variant(RenderPipeline original, boolean shadows, boolean history) {
        ...
        var pipelines = shadows ? history ? HISTORY_PIPELINES : SHADOW_PIPELINES : PIPELINES;
        return pipelines.computeIfAbsent(original, key -> {
            String mode = shadows ? history ? "history_" : "shadow_" : "";
            var builder = key.toBuilder().withLocation(AnvilCraft.of("pipeline/mun_surface_" + mode + pipelines.size()))
            ...
            if (shadows) builder.withShaderDefine("MUN_SHADOWS");
            if (shadows && history) builder.withShaderDefine("MUN_SHADOW_HISTORY");
    ...
    public static void register(RegisterRenderPipelinesEvent event) {
        ...
        for (var original : new RenderPipeline[]{RenderPipelines.SOLID_TERRAIN, RenderPipelines.CUTOUT_TERRAIN,
            ...
            event.registerPipeline(variant(original, false, false));
            event.registerPipeline(variant(original, true, false));
            variant(original, true, true);
        }
    }
    ...
    public static boolean shadowsRequested() {
        return worldPass && profile.cascades() > 0;
    }

    public static void onBlockChanged(net.minecraft.client.multiplayer.ClientLevel level, BlockPos pos,
                                      net.minecraft.world.level.block.state.BlockState previous,
                                      net.minecraft.world.level.block.state.BlockState state) {
        if (requested && profile.cascades() > 0 && level == Minecraft.getInstance().level) {
            SHADOWS.map.blockChanged(level, pos, previous, state);
        }
    }

    public static void onModelChanged(net.minecraft.client.multiplayer.ClientLevel level, BlockPos pos) {
        if (!requested || profile.cascades() == 0 || level != Minecraft.getInstance().level) return;
        for (int z = (pos.getZ() - 1) >> 4; z <= (pos.getZ() + 1) >> 4; z++) {
            for (int x = (pos.getX() - 1) >> 4; x <= (pos.getX() + 1) >> 4; x++) {
                SHADOWS.map.chunkChanged(new net.minecraft.world.level.ChunkPos(x, z));
            }
        }
    }

    public static void onChunkChanged(net.minecraft.client.multiplayer.ClientLevel level, net.minecraft.world.level.ChunkPos pos) {
        if (requested && profile.cascades() > 0 && level == Minecraft.getInstance().level) SHADOWS.map.chunkChanged(pos);
    }

    public static boolean bindShadows(GlRenderPipeline pipeline) {
        if (!REPLACEMENTS.contains(pipeline.info()) || !pipeline.info().getShaderDefines().flags().contains("MUN_SHADOWS")) return true;
        if (!worldPass || profile.cascades() == 0) return false;
        try {
            SHADOWS.bind(pipeline);
            return true;
        } catch (RuntimeException exception) {
            worldPass = false;
            SHADOWS.close();
    ...
    private record Frame(long time, double partialTime, float partialTick, Vec3 camera, Vec3 anchor,
                         org.joml.Matrix4f projection, boolean ambientOcclusion) {
    }
}
