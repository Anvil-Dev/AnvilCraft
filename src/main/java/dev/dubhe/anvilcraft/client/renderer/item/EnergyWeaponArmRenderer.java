...
public final class EnergyWeaponArmRenderer {
    ...
    public static void render(
        PoseStack pose, SubmitNodeCollector buffer, int light, AbstractClientPlayer player, HumanoidArm arm, Matrix4f transform
    ) {
    ...
    private static void renderArm(
        PoseStack pose, SubmitNodeCollector buffer, int light, AbstractClientPlayer player, Matrix4f transform,
        ModelPart.Cube skin, ModelPart.Cube sleeve, boolean showSleeve
    ) {
        pose.pushPose();
        pose.mulPose(transform);
        buffer.submitCustomGeometry(pose, RenderTypes.entitySolid(player.getSkin().body().texturePath()),
            (entry, consumer) -> skin.compile(entry, consumer, light, OverlayTexture.NO_OVERLAY, -1));
        if (showSleeve) {
            buffer.submitCustomGeometry(pose, RenderTypes.entityTranslucent(player.getSkin().body().texturePath()),
                (entry, consumer) -> sleeve.compile(entry, consumer, light, OverlayTexture.NO_OVERLAY, -1));
        }
        pose.popPose();
    }
