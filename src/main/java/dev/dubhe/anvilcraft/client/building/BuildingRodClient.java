...
public final class BuildingRodClient {
    ...
    @Nullable private static List<BuildingPlan.Cell> placementCells;
            ...
            if (disk != null && snapshot == null && !invalidDisk) {
                StructureLoadUtil.getStructureNbtForPreview(mc.level, disk).ifPresent(tag -> {
                    try {
                        ...
                        invalidDisk = true;
                        mc.gui.setOverlayMessage(Component.translatable("message.anvilcraft.building_rod.invalid_structure"), false);
                    }
                });
            }
        }
    ...
    private static void press() {
        ...
        if (snapshot != null) {
            ...
            first = target;
            firstHit = currentHit;
            Minecraft.getInstance().getConnection().send(new BuildingRodPacket(first, first, face, false, Rotation.NONE,
    ...
    private static void confirmSelection() {
        ...
        if (snapshot != null) {
            ...
            Minecraft.getInstance().getConnection().send(new BuildingRodPacket(
                first, target, firstHit == null ? face : firstHit.getDirection(),
    ...
    public static List<BuildingPlan.Cell> placementPreview() {
        var player = Minecraft.getInstance().player;
    ...
    private static boolean canAdjustDistance() {
        ...
        return (first != null || !locked && snapshot != null && !traditional()) && active() && mc.screen == null
            ...
            && (physicalModifiers(mc.getWindow().handle()) & GLFW.GLFW_MOD_CONTROL) != 0;
    }
    ...
    public static boolean handleKeyboardInput(int key, int scanCode, int action, int modifiers) {
        ...
        if (key == GLFW.GLFW_KEY_Z && (modifiers & GLFW.GLFW_MOD_CONTROL) != 0
            ...
            if (action == GLFW.GLFW_PRESS) {
                Minecraft.getInstance().getConnection().send(new BuildingRodPacket(BlockPos.ZERO, BlockPos.ZERO, Direction.UP,
                    false, Rotation.NONE, Mirror.NONE, false, null, BuildingRodPacket.Action.UNDO));
            }
            return true;
        }
        InputConstants.Key pressed = key == GLFW.GLFW_KEY_UNKNOWN ? InputConstants.Type.SCANCODE.getOrCreate(scanCode)
            : InputConstants.Type.KEYSYM.getOrCreate(key);
        return handleControl(pressed, scanCode, action, modifiers);
    }
    ...
    private static boolean modifiersMatch(KeyMapping key, int modifiers) {
        return switch (key.getKeyModifier()) {
            ...
            case CONTROL_OR_COMMAND -> (modifiers & (net.minecraft.client.input.InputQuirks.REPLACE_CTRL_KEY_WITH_CMD_KEY
                ? GLFW.GLFW_MOD_SUPER : GLFW.GLFW_MOD_CONTROL)) != 0;
    ...
    private static boolean physicalKeyDown(InputConstants.Key key, int index, long window) {
        ...
        return switch (key.getType()) {
            case KEYSYM -> code >= GLFW.GLFW_KEY_SPACE && code <= GLFW.GLFW_KEY_LAST && (GLFW.glfwGetKey(window, code) == GLFW.GLFW_PRESS);
    ...
    static void confirm() {
        ...
        BuildingRodItemRenderer.preparePlacement();
        Minecraft.getInstance().getConnection().send(new BuildingRodPacket(
            placement.anchor(), placement.anchor().offset(blueprintOffset), face, true,
    ...
    public static void overlay(RenderGuiEvent.Post event) {
        ...
        for (var line : lines) {
            ...
            event.getGuiGraphics().centeredText(mc.font, line, event.getGuiGraphics().guiWidth() / 2, y, 0xFFFFFFFF);
            y += 10;
        }
    }
