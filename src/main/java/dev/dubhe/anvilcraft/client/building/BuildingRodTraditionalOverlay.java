...
final class BuildingRodTraditionalOverlay {
    ...
    private static final Map<BuildingRodTraditionalControls.Tool, Identifier> ICONS =
    ...
    static void render(GuiGraphicsExtractor graphics) {
        ...
        for (int index = 0; index < tools.length; index++) {
            ...
            graphics.blit(net.minecraft.client.renderer.RenderPipelines.GUI_TEXTURED, ICONS.get(tool),
                left + index * ICON_SPACING, top, 0, frameV,
    ...
    private static void drawLine(GuiGraphicsExtractor graphics, Font font, FormattedCharSequence text, int y) {
        int width = font.width(text);
        int left = (graphics.guiWidth() - width) / 2;
        graphics.fill(left - 3, y - 1, left + width + 3, y + LINE_HEIGHT, BACKGROUND_COLOR);
        graphics.text(font, text, left, y, 0xFFFFFFFF, false);
    }
