...
public class BlockStateRender {
     ...
     * 渲染方块状态
     */
    public static void renderBlock(BlockState state, int posY, @NotNull GuiGraphics draw, float size) {
        ...
        ItemTransform transform =
                new ItemTransform(new Vector3f(30, 210, 0), new Vector3f(0.925f, -0.8125f, 0), new Vector3f(0.625f));
        transform.apply(false, poseStack);
        poseStack.translate(0, posY, 0);
        Minecraft minecraft = Minecraft.getInstance();
        BlockRenderDispatcher blockRenderer = minecraft.getBlockRenderer();
        blockRenderer.renderSingleBlock(
                state, poseStack, minecraft.renderBuffers().bufferSource(), 0xF000F0, OverlayTexture.NO_OVERLAY);
        poseStack.popPose();
        draw.flush();
    }
