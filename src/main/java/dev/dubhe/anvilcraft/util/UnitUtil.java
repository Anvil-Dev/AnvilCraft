...
public class UnitUtil {
    ...
    public static String electricityUnit(int power, boolean original, boolean infinite) {
        ...
        if (power < 1000) return String.format("%d kW", power);
            ...
            if (truncated == Math.floor(truncated)) return String.format("%.0f MW", truncated);
            else return String.format("%.2f MW", truncated);
        } else {
            double gwValue = (double) power / 1000000;
            double truncated = Math.floor(gwValue * 100) / 100;
            if (truncated == Math.floor(truncated)) return String.format("%.0f GW", truncated);
            else return String.format("%.2f GW", truncated);
        }
    }

    public static String electricityUnit(int consume, int generate, boolean original) {
        if (original) return consume + "/" + generate + " kW";
        if (generate < 1000) return String.format("%d/%d kW", consume, generate);
