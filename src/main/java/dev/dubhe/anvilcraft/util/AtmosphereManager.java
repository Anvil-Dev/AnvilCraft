...
public final class AtmosphereManager {
    ...
    static {
        registerDimensionAtmosphere(ModLevelKeys.MUN, Atmosphere.VACUUM);
        registerDimensionAtmosphere(ModLevelKeys.VOID_PLANET, Atmosphere.VACUUM);
        registerDimensionAirResistance(ModLevelKeys.VOID_PLANET, 0.0);
    }
    ...
    /** 26.1 不再提供旧的流体类型浸没缓存，按当前流体交互范围判断自定义流体。 */
    public static boolean isInFluid(Entity entity) {
        if (entity.isInLiquid()) return true;
        AABB bounds = entity.getFluidInteractionBox();
        if (bounds == null) return false;
        return BlockPos.betweenClosedStream(bounds).anyMatch(pos -> {
            var fluid = entity.level().getFluidState(pos);
            return !fluid.isEmpty() && bounds.intersects(pos.getX(), pos.getY(), pos.getZ(),
                pos.getX() + 1, pos.getY() + fluid.getHeight(entity.level(), pos), pos.getZ() + 1);
        });
    }
