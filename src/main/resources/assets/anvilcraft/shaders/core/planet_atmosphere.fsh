...
flat in vec3 CameraLocal;
flat in vec3 LightLocal;
in vec3 AtmosphereColor;
const float Thickness = 0.1875;
in vec3 localPosition;
in float vertexDistance;
in float cylindricalVertexDistance;
in float atmosphereOpacity;
out vec4 fragColor;
...
void main() {
    ...
    fragColor = vec4(haze, opacity * atmosphereOpacity * (1.0 - total_fog_value(vertexDistance, cylindricalVertexDistance,
        FogEnvironmentalStart, FogEnvironmentalEnd, FogRenderDistanceStart, FogRenderDistanceEnd))) * ColorModulator;
}
