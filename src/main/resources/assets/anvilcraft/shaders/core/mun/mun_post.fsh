#version 330
layout(std140) uniform MunPost {
    mat4 InverseProjection;
    vec4 ProjectionAndSceneTexel;
    vec4 TexelAndBlur;
    vec4 EffectSettings;
    vec4 LightSettings;
};
#define ProjectionScale ProjectionAndSceneTexel.xy
#define SceneTexelSize ProjectionAndSceneTexel.zw
#define TexelSize TexelAndBlur.xy
#define BlurDirection TexelAndBlur.zw
#define Mode int(EffectSettings.x)
#define SampleCount int(EffectSettings.y)
#define Radius EffectSettings.z
#define AoStrength EffectSettings.w
#define GlareStrength LightSettings.x
#define Daylight LightSettings.y
...
vec3 viewPosition(vec2 uv, float depth) {
    vec4 point = InverseProjection * vec4(uv * 2.0 - 1.0, LightSettings.z > 0.5 ? depth : depth * 2.0 - 1.0, 1.0);
