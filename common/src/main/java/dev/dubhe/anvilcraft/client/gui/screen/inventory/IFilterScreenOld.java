...
public interface IFilterScreenOld {
    ...
    EnableFilterButton getEnableFilterButton();
    ...
    default boolean isRecord() {
        return this.getEnableFilterButton() != null && this.getEnableFilterButton().getFilterEnabled().get();
    }
    ...
    default BiFunction<Integer, Integer, EnableFilterButton> getEnableFilterButtonSupplier(int x, int y, Supplier<Boolean> filterEnabled) {
        return (i, j) -> new EnableFilterButton(i + x, j + y, button -> {
            if (button instanceof EnableFilterButton button1) {
                MachineEnableFilterPack packet = new MachineEnableFilterPack(button1.next());
                packet.send();
            }
        }, filterEnabled);
    }
