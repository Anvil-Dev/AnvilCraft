"""核对整合器源版模型、纹理、配方和玩家提示。"""
import json
from pathlib import Path

root = Path(__file__).resolve().parents[2]
reference = root / "build/porting/reference-1.21"
for name in ["storage_port_consolidator", "hd_storage_port_consolidator"]:
...
state = "assets/anvilcraft/blockstates/storage_port_consolidator.json"
...
for path in [Path("src/main/resources/assets/anvilcraft/lang/zh_cn.json"),
    ...
    for key, value in source.items():
        if "storage_port_consolidator" in key:
            assert target.get(key) == value, key
path = Path("src/generated/resources/data/anvilcraft/recipe/storage_port_consolidator.json")
...
model = json.loads((root / "src/generated/resources/assets/anvilcraft/items/storage_port_consolidator.json").read_text())["model"]
assert model == {"type": "minecraft:model", "model": "anvilcraft:block/storage_port_consolidator"}
loot = Path("src/generated/resources/data/anvilcraft/loot_table/blocks/storage_port_consolidator.json")
assert json.loads((root / loot).read_text()) == json.loads((reference / loot).read_text())
print("Consolidator: source models, textures, variants, translations, recipe, loot and native item definition verified")
