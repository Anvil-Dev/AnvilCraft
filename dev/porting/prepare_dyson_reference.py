"""Install the same fixed Dyson/red-dwarf world fixture in the source worktree."""
...
relative = "dev/dubhe/anvilcraft/porting/DysonSphereClientScene.java"
...
scene = scene.replace("        DysonSphereRendererChecks.verify(be, sample.brown, sample.special, sample.amplifier, sample.sphere);\n", "")
scene = scene.replace("dyson-26.1-", "dyson-1.21-")
...
text = wrapper.read_text(encoding="utf-8").replace("CelestialAnvilItemClientScene.frame(client)", "DysonSphereClientScene.frame(client)")
